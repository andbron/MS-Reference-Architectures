...
HUB_NETWORK_RESOURCE_GROUP_OUTPUT=$(azure group create --name $RESOURCE_GROUP_NAME --location $LOCATION --subscription $SUBSCRIPTION_ID --json) || exit 1

# Create the hub virtual network
echo "Deploying hub virtual network..."
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name "ra-hub-vnet-deployment" \
...
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name "ra-hub-jb-deployment" \
...
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name "ra-hub-vpn-deployment" \
