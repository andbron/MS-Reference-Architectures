...
RESOURCE_GROUP_NAME="ra-sap-hana-rg"
LOCATION="westus"

TEMPLATE_ROOT_URI=${TEMPLATE_ROOT_URI:="https://raw.githubusercontent.com/mspnp/template-building-blocks/v1.0.0/"}
...
LOAD_BALANCED_VM_TEMPLATE_URI="${TEMPLATE_ROOT_URI}templates/buildingBlocks/loadBalancer-backend-n-vm/azuredeploy.json"
...
MGMT_TIER_JUMPBOX_PARAMETERS_PATH="${SCRIPT_DIR}/parameters/managementTierJumpbox.parameters.json"
SAP_APPS_TIER_PARAMETERS_PATH="${SCRIPT_DIR}/parameters/sapAppsTier.parameters.json"
SAP_APPS_TIER_SCS_PARAMETERS_PATH="${SCRIPT_DIR}/parameters/sapAppsTier.scs.parameters.json"
SAP_DATA_TIER_PARAMETERS_PATH="${SCRIPT_DIR}/parameters/sapDataTier.parameters.json"

VIRTUAL_NETWORK_DEPLOYMENT_NAME="ra-sap-hana-vnet-deployment"
VIRTUAL_NETWORK_GATEWAY_DEPLOYMENT_NAME="ra-sap-hana-vpn-gateway-deployment"
MGMT_JUMPBOX_DEPLOYMENT_NAME="ra-sap-hana-mgmt-jumpbox-deployment"
SAP_APPS_DEPLOYMENT_NAME="ra-sap-hana-apps-deployment"
SAP_APPS_SCS_DEPLOYMENT_NAME="ra-sap-hana-apps-scs-deployment"
SAP_HANA_DB_DEPLOYMENT_NAME="ra-sap-hana-db-deployment"
...
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name $MGMT_JUMPBOX_DEPLOYMENT_NAME \
--template-uri $VIRTUAL_MACHINE_TEMPLATE_URI --parameters-file $MGMT_TIER_JUMPBOX_PARAMETERS_PATH \
--subscription $SUBSCRIPTION_ID || exit 1

# Create the Apps/NetWeaver tier
echo "Deploying SAP Apps tier..."
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name $SAP_APPS_DEPLOYMENT_NAME \
--template-uri $LOAD_BALANCED_VM_TEMPLATE_URI --parameters-file $SAP_APPS_TIER_PARAMETERS_PATH \
--subscription $SUBSCRIPTION_ID || exit 1

# Create the Apps/SCS tier
echo "Deploying SAP Apps (SCS) tier..."
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name $SAP_APPS_SCS_DEPLOYMENT_NAME \
--template-uri $LOAD_BALANCED_VM_TEMPLATE_URI --parameters-file $SAP_APPS_TIER_SCS_PARAMETERS_PATH \
--subscription $SUBSCRIPTION_ID || exit 1

# Create the Hana DB
echo "Deploying SAP Hana DB tier..."
azure group deployment create --resource-group $RESOURCE_GROUP_NAME --name $SAP_HANA_DB_DEPLOYMENT_NAME \
--template-uri $VIRTUAL_MACHINE_TEMPLATE_URI --parameters-file $SAP_DATA_TIER_PARAMETERS_PATH \
--subscription $SUBSCRIPTION_ID || exit 1
...
azure group deployment show --resource-group $RESOURCE_GROUP_NAME --name $MGMT_JUMPBOX_DEPLOYMENT_NAME \
--subscription $SUBSCRIPTION_ID --json || exit 1

azure group deployment show --resource-group $RESOURCE_GROUP_NAME --name $SAP_APPS_DEPLOYMENT_NAME \
--subscription $SUBSCRIPTION_ID --json || exit 1

azure group deployment show --resource-group $RESOURCE_GROUP_NAME --name $SAP_APPS_SCS_DEPLOYMENT_NAME \
--subscription $SUBSCRIPTION_ID --json || exit 1

azure group deployment show --resource-group $RESOURCE_GROUP_NAME --name $SAP_HANA_DB_DEPLOYMENT_NAME \
--subscription $SUBSCRIPTION_ID --json || exit 1
